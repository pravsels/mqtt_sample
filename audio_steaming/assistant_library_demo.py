...
if __name__ == '__main__':
    ...
    assistant = AssistantServiceClient(volume_percentage=args.volume,
                                       language_code=args.language)

    Thread(target=main, args=(args, assistant)).start()

    port = 3000
    print("Working on port: ", port)

    reactor.listenUDP(port, assistant)
    reactor.run()
